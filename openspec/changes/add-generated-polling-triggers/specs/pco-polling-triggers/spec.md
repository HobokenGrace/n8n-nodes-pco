...
Each generated product trigger SHALL expose separate Resource and Event controls. The Resource control SHALL group polling operations by the same generated resource types as action operations. Event labels SHALL be concise and SHALL preserve nested collection scope only where needed to distinguish routes. The corresponding n8n action labels SHALL start with `On` and include the resource name so operations remain identifiable outside the configured node. Each `Created` option SHALL describe that it follows creation time and does not fire when an older resource merely starts matching. Each `Created or Updated` option SHALL describe that it includes initial creation and later changes whose update timestamp advances.
...
- **THEN** the Resource control SHALL contain `<Resource Type>`
- **AND** its Event control SHALL use `Created (via <Scope>)` or `Created or Updated (via <Scope>)`
...
- **AND** the first scheduled poll SHALL return matching records from that boundary in oldest-first order
- **AND** it SHALL apply Max Records Per Poll to that first scheduled batch
- **AND** later scheduled polls SHALL continue any remaining backlog from the persisted boundary
...
- **THEN** reactivation SHALL preserve the saved watermark and return no resource items
- **AND** the first and later configured Poll Times SHALL process matching changes that occurred while inactive, one capped batch per Poll Time
