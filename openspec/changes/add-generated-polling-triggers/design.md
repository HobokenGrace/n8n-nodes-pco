...
The OpenAPI parser will derive a polling operation only from a GET operation modeled as a collection whose `order` values contain the cursor field and whose parameters contain `where[<cursor>][gte]` with date-time semantics. The generated node uses separate Resource and Event controls. `created_at` produces a `Created` Event whose description explains that an older resource merely entering the filtered result set does not qualify. `updated_at` produces `Created or Updated` because PCO initializes `updated_at` on creation; its description explains that both initial creation and later timestamp-advancing changes qualify. Nested routes append `(via <Scope>)` to the Event label only where route context is needed. The Event selector keeps these concise labels, while n8n action metadata includes the resource and starts with `On`, such as `On Person created`, so actions remain identifiable in the node-selection panel and follow trigger naming conventions. At runtime, every primary resource must provide a non-empty string JSON:API `type`, non-empty string `id`, and valid cursor value; response validation supplies the invariant when a snapshot does not model resource attributes precisely.
...
n8n persists polling static data after activation or after a scheduled poll returns a non-null response. Every activation initializes and persists its applicable boundary without returning resource items. With an explicit Start Time, the first configured Poll Time begins catch-up rather than activation bypassing the user's schedule. If a scheduled fingerprint change or other state-only transition produces no resource items, the poll returns one empty output branch (`[[]]`) so n8n persists the new state. This intentionally creates a successful visible workflow execution with zero items; downstream nodes receive nothing, and user documentation distinguishes it from both a resource event and a trigger failure. Returning `null` is reserved for scheduled polls that neither emit resources nor change durable state.

Ordinary workflow deactivation preserves state. Reactivation with the same fingerprint returns no resource items during activation, then resumes from the saved watermark at the first configured Poll Time and catches up changes that occurred while inactive one capped batch per Poll Time. This avoids silent loss during maintenance or deployment downtime. A copied workflow or node without inherited static state initializes independently.

### Default to new changes only

An empty Start Time is presented as new changes only. Its user-facing description makes clear that existing matching records are not emitted. Initialization requests the newest matching resource using descending cursor order and a one-record page. If a match exists, the runtime records its timestamp and obtains every matching `(type, id)` identity at that exact boundary timestamp so pre-existing ties are also ignored. If no match exists, it records `1970-01-01T00:00:00.000Z` with no seen identities. This fixed sentinel avoids worker-clock skew and delayed-visibility gaps; a pre-activation resource that becomes visible or starts matching only after initialization can therefore be emitted. Initialization emits no records.

Start Time uses n8n's native date-time control with an empty default and expression support, rather than requiring users to hand-format a string. A configured or expression-resolved value must be a valid RFC 3339 date-time with an explicit offset and is canonicalized to UTC. It bypasses newest-record baselining and initializes an inclusive watermark at that instant with no seen identities. Activation persists that boundary without resource output. If the start is at or before activation time, the first configured Poll Time begins processing matching history in oldest-first batches capped by Max Records Per Poll; later scheduled polls continue any backlog. Future values are intentionally valid and are neither rejected nor clamped: the workflow remains active, but scheduled polls return no resource items until PCO exposes a matching record whose cursor is at or after that instant. The Start Time description explains this quiet waiting state.
